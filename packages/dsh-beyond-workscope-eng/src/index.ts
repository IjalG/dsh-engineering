...
export function apply(ctx: Context, config?: Config): void {
  ...
  const loader = ctx.get('loader') as { entries?: () => ReadonlyArray<{ options: { id?: string } }> } | undefined
  const webUiOwns = loader?.entries?.().some(entry => entry.options.id === 'beyond-workscope') ?? false
  if (webUiOwns) {
    // Provider marker: the browser half uses this route to tell that THIS
    // copy (the engineering provider) is the one serving.
    ctx.effect(() => {
      const dispose = ctx.get('webServer')?.register?.({
        kind: 'exact',
        path: '/api/dsh-beyond-workscope-eng/ping',
        handler: (req: unknown, res: { writeHead: (s: number, h: Record<string, string>) => void; end: (b: string) => void }) => {
          res.writeHead(200, { 'content-type': 'application/json' })
          res.end(JSON.stringify({ provider: 'dsh-web-ui' }))
        },
      })
      return () => { dispose?.() }
    }, 'dsh-beyond-workscope-eng: provider marker')
    return
  }
  ctx.effect(() => {
    const dispose = ctx.get('webServer')?.register?.({
      ...
      handler: (req: unknown, res: { writeHead: (s: number, h: Record<string, string>) => void; end: (b: string) => void }) => {
        res.writeHead(200, { 'content-type': 'application/json' })
        res.end(JSON.stringify({ provider: 'dsh-engineering' }))
      },
    })
    return () => { dispose?.() }
  }, 'dsh-beyond-workscope-eng: provider marker')
