...
## 11. Office 完善路线图（goal-81ee4e69，参照 Office 365 体验 / LibreOffice 可用标准）

### Word（TipTap 生态）
- [x] 富文本编辑 + 格式工具栏（标题/粗斜下划删除线/列表/引用/分隔线/撤销重做）
- [x] docx<->HTML 往返（mammoth/docx，含真实列表与有序编号）
- [ ] 表格编辑（Tiptap Table：插入/增删行列/合并单元格/表头切换）
- [ ] 图片插入（Tiptap Image，本地文件->base64，docx 保存图片）
- [ ] 查找替换、字数统计
- [ ] 页面设置（纸张/方向/页边距）与打印导出（LibreOffice）
- [ ] 样式体系（正文/标题样式集）与最近文件/模板

### Excel（exceljs + hyperformula）
- [x] 网格编辑、行列管理、列宽拖拽、多 sheet
- [ ] 公式引擎（hyperformula：常用函数 + 依赖重算 + fx 栏 + 自动求和）
- [ ] 单元格格式（数字/日期/货币/百分比）
- [ ] 排序/筛选、合并单元格、冻结行列
- [ ] 轻量图表（条形/折线，SVG 自绘）
- [ ] xlsx 公式往返（保存 formula + 缓存值）
...
### PDF（pdf-lib + pdftoppm + LibreOffice）
- [x] 预览（页级 PNG）、合并/拆分、Office 转 PDF、多页 OCR
- [ ] 文本提取、页码水印、书签目录

### 通用
- [ ] 打开/另存对话框（工作区文件选择器复用 nas 文件列表）
- [ ] 最近文件、快捷键体系
